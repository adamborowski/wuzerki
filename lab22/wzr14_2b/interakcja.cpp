...
float oddalenie = 332;                          // oddalenie widoku z kamery
float kat_kam_z = 0- PI*5/180*7;                            // obrót kamery góra-dó³
	...
	case WM_MOUSEMOVE:
		{
			...
			if (sterowanie_myszkowe)
			{
				float kat_skretu = (float)(kursor_x - x)/10;
				if (kat_skretu > 80) kat_skretu = 80;
				if (kat_skretu < -80) kat_skretu = -80;	
