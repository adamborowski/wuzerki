...
void RysujScene()
{
  ...
  GLfloat KolorCudzychObiektow[] = { 0.1f, 0.6f, 0.1f, 0.5f};
  GLfloat RedSurface[] = { 0.8f, 0.2f, 0.1f, 0.5f};
  GLfloat KolorTerenu[] = { 0.7f, 0.5f, 0.0f, 1.0f};
  ...
  glPrint("V = %f, A = %f, V_kat = %f, A_kat = %f",pMojObiekt->wV.dlugosc(), pMojObiekt->wA.dlugosc(),
  pMojObiekt->wV_kat.dlugosc(), pMojObiekt->wA_kat.dlugosc());  
  */
  ...
  glDisable(GL_BLEND);
  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, KolorTerenu);
...
void ZmianaRozmiaruOkna(int cx,int cy)
{
  ...
  glLoadIdentity();
  gluPerspective(55, aspect, 1, 10000.0);
