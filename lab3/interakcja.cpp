...
// Parametry widoku:
Wektor3 kierunek_kamery = Wektor3(11,-3,-11);   // kierunek patrzenia
...
float oddalenie = 1.0;     // oddalenie widoku z kamery
...
float czas_odnowy_przedm = 100;      // czas w [s] po którym wziête przedmioty odnawiaj¹ siê
...
void PoczatekInterakcji()
{
	...
	multi_reciv = new multicast_net("224.12.12.136",10001);      // obiekt do odbioru ramek sieciowych
	multi_send = new multicast_net("224.12.12.136",10001);       // obiekt do wysy³ania ramek
	...
	case WM_LBUTTONDOWN: //reakcja na lewy przycisk myszki
		{
			...
			if (sterowanie_myszkowe)
				pMojObiekt->F = 4000.0;        // si³a pchaj¹ca do przodu
			break;
		}
	case WM_RBUTTONDOWN: //reakcja na prawy przycisk myszki
		{
			...
			int y = HIWORD(lParam);
			if (sterowanie_myszkowe)
				pMojObiekt->F = -2000.0;        // si³a pchaj¹ca do tylu
			break;
		}
	...
	case WM_KEYDOWN:
		{
			...
			case VK_DOWN:
				{
					pMojObiekt->F = -2000.0;        // sila pchajaca do tylu
					break;
				}
			case VK_LEFT:
				{
			...
			case 'F':  // przekazanie 10 kg paliwa pojazdowi, który znajduje siê najbli¿ej
				{
					...
					if (ilosc_p == 0) 
						MessageBox(okno,"Paliwa nie da³o siê przekazaæ, bo byæ mo¿e najbli¿szy pojazd znajduje siê zbyt daleko.",
			...
			case 'G':  // przekazanie 100 jednostek gotowki pojazdowi, który znajduje siê najbli¿ej
				{
					...
					if (ilosc_p == 0) 
						MessageBox(okno,"Gotówki nie da³o siê przekazaæ, bo byæ mo¿e najbli¿szy pojazd znajduje siê zbyt daleko.",
