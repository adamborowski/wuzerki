...
unsigned long IndeksyObiektow[2000];                // tablica indeksow innych obiektow ulatwiajaca wyszukiwanie
...
DWORD WINAPI WatekOdbioru(void *ptr)
{
  ...
  StanObiektu stan;
	...
	while(1)
	{
		...
		rozmiar = pmt_net->reciv((char*)&ramka,&lastIP, sizeof(StanObiektu));   // oczekiwanie na nadejœcie ramki 
		stan = ramka.stan;
                                            ...
                                            // stworzyæ
      {
        ...
        IndeksyObiektow[stan.iID] = lastIP;     // wpis do tablicy indeksowanej numerami ID
	  ...
	  {
		  if(IndeksyObiektow[i] != -1)
			  multi_send->send((char*)&ramka,IndeksyObiektow[i], sizeof(Ramka));
	  }
	}  // koniec while(1)
	return 1;
}
...
void PoczatekInterakcji()
{
   ...
   multi_reciv = new unicast_net(10000);      // obiekt do odbioru ramek sieciowych
   multi_send = new unicast_net(10000);       // obiekt do wysy³ania ramek
