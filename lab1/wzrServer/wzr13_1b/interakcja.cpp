...
void PoczatekInterakcji()
{
   ...
   multi_reciv = new unicast_net(1001);      // obiekt do odbioru ramek sieciowych
   multi_send = new unicast_net(1002);       // obiekt do wysy³ania ramek
...
void Cykl_WS()
{
   ...
   int iRozmiar = multi_send->send((char*)&ramka,serverAddr, sizeof(Ramka));          
}
  ...
  switch (kod_meldunku) 
    ...
    case WM_KEYDOWN:
        ...
        case 'S':   // przybli¿enie widoku
        {
          ...
          {
            kierunek_kamery = Wektor3(5,-3,-14);   // kierunek patrzenia
            pol_kamery = Wektor3(-50,7,10);          // po³o¿enie kamery
            pion_kamery = Wektor3(0,1,0);           // kierunek pionu kamery  
          }
					break;
				}
