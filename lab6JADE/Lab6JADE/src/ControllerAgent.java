...
// --------------------------------------------
   ...
   int stany_agentow[][] = new int[100][3]; // stany agentow mobilnych
		...
		if (msg!=null)
		    ...
		    //System.out.println("Otrzymałem od agenta nr "+numer_agenta+" ("+  msg.getSender().getName()+") numer akcji = "+akcja);
                    if (akcja == -1)  // agent chce by wylosować mu stan początkowy
                    {
                        ...
                        //        stany_agentow[liczba_agentow][0]+","+stany_agentow[liczba_agentow][1]+")");
                        b = new byte[4];
                        for (int i=0; i < 2; i++) b[i] = (byte)stany_agentow[liczba_agentow][i];   // stan
                        b[2] = 0;                                                                  // nagroda
                        b[3] = (byte)liczba_agentow;                                               // numer agenta
                    ...
                    else              // agent chce uzyskać stan oraz nagrodę po wykonaniu akcji
                    {
                        ...
                        //        stany_agentow[numer_agenta][0]+","+stany_agentow[numer_agenta][1]+
                        //        ") moze wykonac akcję "+akcja);
                        ...
                        //        stan_i_nagroda[0]+","+stan_i_nagroda[1]+")");
                        b = new byte[4];
                        for (int i=0; i < 3; i++) b[i] = (byte)stany_agentow[numer_agenta][i];
                        b[3] = (byte)numer_agenta;
                        list_o_stanie_agenta.setByteSequenceContent(b);
                        list_o_stanie_agenta.setConversationId("Info_o_stanie");
                        send(list_o_stanie_agenta);
                    }
		...
		if (msg!=null)
                {
                    ...
                    byte[] b = new byte[4];
                    for (int i=0; i < 3; i++) b[i] = (byte)stany_agentow[numer_agenta][i];   // stan
                    b[3] = (byte) numer_agenta;
                    reply.setByteSequenceContent(b);
