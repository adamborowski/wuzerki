...
public class PanelPlanszy extends JPanel{// implements Runnable{
    ...
    int moj_stan[];   // stan agenta (numery wiersza i kolumny)
    int moja_akcja;   // akcja agenta w tym stanie
    int moj_numer;

    int stany_agentow[][]={}; // stany innych agentow
    ...
    public void paintComponent(Graphics g)  // metoda odrysowująca panel
    {
        ...
        g2.setColor(new Color(0,0,255));
        g2.fillRect(moj_stan[1]*rozm_pola+m, moj_stan[0]*rozm_pola+m, rozm_pola-2*m, rozm_pola-2*m);  // własny obiekt
        g2.setColor(new Color(255,255,255));
        String st = new String(""+moj_numer);
        g2.drawString(st, moj_stan[1]*rozm_pola+m*3,  moj_stan[0]*rozm_pola+m*4);
        ...
        if (moja_akcja > 0)
        {
            ...
            if (moja_akcja == 1)
                g2.fillOval(moj_stan[1]*rozm_pola+rozm_pola-m, moj_stan[0]*rozm_pola+rozm_pola/3, m*2, m*2);
            else if (moja_akcja == 2)
                g2.fillOval(moj_stan[1]*rozm_pola+rozm_pola/3, moj_stan[0]*rozm_pola+rozm_pola-m, m*2, m*2);
            else if (moja_akcja == 3)
                g2.fillOval(moj_stan[1]*rozm_pola-m, moj_stan[0]*rozm_pola+rozm_pola/3, m*2, m*2);
            else if (moja_akcja == 4)
                g2.fillOval(moj_stan[1]*rozm_pola+rozm_pola/3, moj_stan[0]*rozm_pola-m, m*2, m*2);
        ...
        for (int i=0;i<liczba_agentow;i++)
            if (i != moj_numer)
            {
               ...
               g2.fillRect(stany_agentow[i][1] *rozm_pola+m, stany_agentow[i][0]*rozm_pola+m, rozm_pola-2*m, rozm_pola-2*m);
               g2.setColor(new Color(255,255,255));
               st = new String(""+i);
               g2.drawString(st, stany_agentow[i][1]*rozm_pola+m*3, stany_agentow[i][0]*rozm_pola+m*4);
            }
        g2.setColor(new Color(0,0,0));
